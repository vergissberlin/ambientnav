...
void main() {
  ...
  testWidgets(
    'shows a blur slider only after enabling the camera background',
    (tester) async {
      ...
      expect(
        find.byKey(const Key('cameraNavBackgroundBlurSlider')),
        findsNothing,
      );
      ...
      final slider = find.byKey(const Key('cameraNavBackgroundBlurSlider'));
      expect(slider, findsOneWidget);

      final context = tester.element(find.byType(SettingsScreen));
      final container = ProviderScope.containerOf(context, listen: false);
      expect(
        container.read(cameraBackgroundBlurProvider),
        closeTo(18.0, 0.001),
      );
      ...
      expect(container.read(cameraBackgroundBlurProvider), greaterThan(18.0));
    },
  );
}
